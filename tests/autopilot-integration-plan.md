...
| 13 | Resume after crash | `/bbs:autopilot` (no args, after an earlier session died mid-workflow) | `checkpoint.json` present, status=done_step | Parse=Resume via the ticket's `checkpoint.json`, dispatch=same workflow from next step | announces step ≠ last checkpoint's step |
