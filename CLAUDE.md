...
5. **Git-flow protocol in the skill** — git-flow `mode:` branching and
   ...
   runs the shared-surface protocol (`bbs ticket surface` acquire/compose/revert/release) itself
...
- **Prefer `/bbs:autopilot <workflow> <ticket>` over hand-chaining skills** whenever one ticket needs >1 heavy skill, or when the user's ask could be "build this whole thing." Inline free-text (`/bbs:autopilot <one-line requirement>`) routes to the `builder` workflow, which creates the ticket, seeds `requirement.md`, and picks its own one-ticket mode (child / implement / build / verify) from ticket state. A decomposed parent routes to `foreman`; autopilot never orchestrates its children.
...
reviews, artifact requirements, holds, safety floors and finish policy still apply.
...
"Workspace" names three different things here: an **Orca terminal** (a visible
worker's tab), the **worktree pool** (`.babysit/worktrees/<ticket>_<slug>`),
...
name ("workspace acme"), meaning 1 always carries the `Orca` prefix, and
