...
2. **Plan accepted** → `plan.md` on the ticket. Owner: autopilot init via `plan-draft`; user-facing work routes through `design-ui` inside it, so the plan carries the UI spec + prototype **before** the `/goal` handoff — the handoff is where design is reviewed ahead of implementation (builder build mode covers the case init didn't seed it); stops at `--stop-after=plan` if requested. **This is the one checkpoint a human can delegate.** By default a human holds it, but `bbs foreman grant <id>` hands it to a foreman durably (the grant is a `grant:` block on `~/.babysit/foremen/<id>.yaml`, naming its granter and time), so an overnight or cron-driven batch doesn't stall on the first worker that adds a component. A grant is bounded (`--hours`, `--max`, `--tickets`; `--unbounded` must be typed), revocable (`grant revoke`, effective at the next checkpoint — nothing is rolled back), and changes only *who resolves* the checkpoint: the foreman writes the same approval-record verdict the dashboard writes, via `bbs ticket approval self-resolve`, after filling the same rubric with named evidence and logging it to `decisions.jsonl`. Two things no grant reaches: money, auth and irreversible-data paths always escalate, and a rubric that can't be filled ends in `BLOCKED` with the gaps named rather than an approval or a wait.
