...
Each worker runs in a foreman-created ticket worktree (`bbs ticket ensure --mode=worktree` before dispatch) and initially stops after a scoped plan for coordinator review. Local tests/QA run in the ticket worktree: the primary has unrelated dirty changes and must not be reset or composed onto. A missing runnable browser target uses the real skill's named local fallback. Each worker owns its commits, persisted review/QA verdicts, and checkpoints; foreman applies the repository finish policy and primary landing stays with the human.
