...
| Command | Purpose |
|---------|---------|
| `bbs autopilot` | State helpers the `/bbs:autopilot` skill uses, also runnable by hand for debugging: `probe` (dump probed state), `explain` (show recommended workflow; add `--details` for the per-workflow PASS/FAIL table), `base-branch` (resolve with per-project override), `lint-workflow <path>` (authoring-time `needs-state:` lint), `spawn-goal` (what the skill's `--auto` runs: `/goal` on the start agent), `spawn-review --agent <name>` (what the skill's `--reviewer` runs; add `--builder <agent>` so approve starts `/goal`), `spawn-verify` (what the skill's `--verify` runs: review-pr + qa in a process that never saw the diff being written, reporting only through the verdict files it persists — `--agent <name>` to grade on a different model), plus the checkpoint surface `read` / `checkpoint` / `timeline` / `recover` / `clear` / `current` |
